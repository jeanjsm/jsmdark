...
# Local (package-relative) imports
from ..models.models import ConfigModel, QueueItem, ChromaConfig
from ..ui.ui_main_window import VideoGeneratorGUI
from ..services.threads import ProcessVideoThread, QueueWorkerThread
from ..ui.ui_tabs import FileBrowseWidget, ChromaItemWidget
...
class AppController(QObject):
    ...
    def update_model_from_ui(self):
        for field_name in self.model.__annotations__:
            # Skip fields managed specially
            if field_name in ["opening_video_paths", "chroma_list"]:
                continue
            # Handle optional camera_shake_config only if present on model
            if field_name == "camera_shake_config":
                config = getattr(self.model, "camera_shake_config", None)
                if config is None:
                    continue
                config.enabled = self.view.subtitle_tab.camera_shake_enabled.isChecked()
                config.intensity = self.view.subtitle_tab.camera_shake_intensity.value()
                config.frequency = self.view.subtitle_tab.camera_shake_frequency.value()
                config.duration = self.view.subtitle_tab.camera_shake_duration.value()
                continue
