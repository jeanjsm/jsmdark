...
class ConfigModel:
    ...
    def save(self, filepath=None):
        ...
        Args:
            filepath (str, optional): Path to save the config file. If None, uses the project root config.json.
        """
        if filepath is None:
            filepath = Path(__file__).parent.parent.parent / "config.json"
    ...
    def load(cls, filepath=None):
        ...
        Args:
            filepath (str, optional): Path to the config file. If None, uses the project root config.json.

        Returns:
            ConfigModel: Loaded configuration or a default instance if the file doesn't exist.
        """
        if filepath is None:
            filepath = Path(__file__).parent.parent.parent / "config.json"
        ...
        try:
            with open(filepath, "r", encoding='utf-8') as f:
                ...
                # Cria a instância com os dados filtrados
                return cls(**filtered_data)
        except Exception as e:
            print(f"Erro ao carregar configuração: {e}")
            return cls()  # Retorna configuração padrão em caso de erro
