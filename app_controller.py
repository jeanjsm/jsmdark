...
from PySide6.QtWidgets import (QMessageBox, QListWidgetItem, QLineEdit, QCheckBox, QSpinBox, QDoubleSpinBox, QComboBox,
                               QFileDialog)
...
class AppController(QObject):
    ...
    def connect_signals(self):
        ...
        self.view.video_tab.remove_opening_button.clicked.connect(self._remove_opening_video)
        self.view.overlay_tab.add_chroma_btn.clicked.connect(lambda: self._add_chroma_widget())
    ...
    def add_to_queue(self):
        ...
        if any(item.narration_path == self.model.narration_path for item in self.queue_items):
            ...
            return
        item = QueueItem(narration_path=self.model.narration_path, output_path=output_path)
    ...
    def update_model_from_ui(self):
        for field_name in self.model.__annotations__:
            if field_name in ['opening_video_paths', 'chroma_list']: continue
            if field_name == 'camera_shake_config':
                self.model.camera_shake_config.enabled = self.view.subtitle_tab.camera_shake_enabled.isChecked()
                self.model.camera_shake_config.intensity = self.view.subtitle_tab.camera_shake_intensity.value()
                self.model.camera_shake_config.frequency = self.view.subtitle_tab.camera_shake_frequency.value()
                self.model.camera_shake_config.duration = self.view.subtitle_tab.camera_shake_duration.value()
                continue
    ...
    def _find_widget_for_field(self, field_name: str):
        ...
        for tab in [self.view.basic_tab, self.view.video_tab, self.view.overlay_tab, self.view.subtitle_tab]:
            if hasattr(tab, field_name):
                return getattr(tab, field_name)
        return None

    def _add_chroma_widget(self, config: ChromaConfig = None):
        if config is None: config = ChromaConfig()
    ...
    def _update_chroma_list_from_ui(self):
        ...
        for widget in self.chroma_widgets:
            self.model.chroma_list.append(ChromaConfig(path=widget.path.text(), scale=widget.scale.value(),
                                                       position=widget.position.currentText(),
                                                       start=widget.start.value()))

    def _update_ui_from_chroma_list(self):
        for widget in self.chroma_widgets: widget.deleteLater()
        self.chroma_widgets.clear()
        for config in self.model.chroma_list: self._add_chroma_widget(config)

    def _on_single_process_finished(self, success, message):
        self._set_ui_enabled(True)
        self.view.progress_bar.setValue(100 if success else 0)
        if success:
            QMessageBox.information(self.view, "Sucesso", message); self.log_message(f"✅ Sucesso: {message}")
        else:
            QMessageBox.critical(self.view, "Erro", message); self.log_message(f"❌ Erro: {message}")

    def stop_queue_processing(self):
        if self.queue_worker and self.queue_worker.isRunning(): self.queue_worker.stop(); self.queue_worker.wait()
        self._on_queue_finished(interrupted=True)

    def clear_queue(self):
        if self.is_processing_queue: return
        if QMessageBox.question(self.view, "Confirmar", "Limpar toda a fila?",
    ...
    def remove_selected_from_queue(self):
        ...
        if current_row < 0 or self.queue_items[current_row].status == "processing": return
    ...
    def _on_queue_item_started(self, index):
        if index < len(self.queue_items): self.queue_items[
            index].status = "processing"; self._update_queue_display(); self.log_message(
            f"Iniciando item {index + 1}/{len(self.queue_items)}...")

    def _on_queue_item_progress(self, index, percent):
        processed_count = sum(1 for item in self.queue_items if item.status == "completed")
        total_progress = processed_count * 100
        if index < len(self.queue_items) and self.queue_items[index].status == "processing": total_progress += percent
        num_items = len(self.queue_items)
        self.view.progress_bar.setValue(int(total_progress / num_items if num_items > 0 else 0))
    ...
    def _on_queue_item_finished(self, index, success, message):
        if index < len(self.queue_items):
            ...
            self._update_queue_display();
            self.log_message(f"✅ Item {index + 1} concluído." if success else f"❌ Erro no item {index + 1}: {message}")

    def _on_queue_finished(self, interrupted=False):
        self.is_processing_queue = False;
        self._set_ui_enabled(True)
        if interrupted: self.log_message("Processamento da fila interrompido."); return
        ...
        if len(self.queue_items) > 0 and errors == 0: self.view.progress_bar.setValue(100)

    def _update_queue_display(self):
        self.view.queue_list.clear();
        status_map = {"waiting": "🟡", "processing": "🔵", "completed": "🟢", "error": "🔴"}
        for item in self.queue_items: self.view.queue_list.addItem(
    ...
    def _set_ui_enabled(self, enabled: bool):
        ...
        if not enabled: self.view.progress_bar.setValue(0)

    def _validate_inputs(self) -> bool:
        self.update_model_from_ui()
        if not self.model.narration_path or not Path(self.model.narration_path).exists(): QMessageBox.warning(self.view,
                                                                                                              "Entrada Inválida",
                                                                                                              "Selecione um arquivo de narração válido."); return False
        if not self.model.videos_folder or not Path(self.model.videos_folder).is_dir(): QMessageBox.warning(self.view,
                                                                                                            "Entrada Inválida",
                                                                                                            "Selecione uma pasta de mídia válida."); return False
        if not self.model.output_folder: QMessageBox.warning(self.view, "Entrada Inválida",
                                                             "Selecione uma pasta de destino válida."); return False
        try:
    ...
    def _add_opening_video(self):
        path, _ = QFileDialog.getOpenFileName(self.view, "Selecionar Vídeo de Abertura", "", "Vídeos (*.mp4 *.mov)")
        if path and path not in self.model.opening_video_paths: self.model.opening_video_paths.append(
            path); self._update_opening_videos_display()
    ...
    def _remove_opening_video(self):
        ...
        if path_to_remove in self.model.opening_video_paths: self.model.opening_video_paths.remove(
            path_to_remove); self._update_opening_videos_display()

    def _update_opening_videos_display(self):
        widget = self.view.video_tab.opening_video_paths_widget;
        widget.clear()
        for path in self.model.opening_video_paths: widget.addItem(path)
